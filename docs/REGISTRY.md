...
1. **The clip is read locally.** Frames are extracted by ffmpeg on the local computer, sampled evenly
...
- **The measurement is the headline; the fit diagnostics are behind the mode switch.** Viewport
  ...
  quotes no error; when the upstream fit reports failure or staleness, it refuses instead of quoting
  a number (`app/src/panes/result-strip.ts`, 9 tests). The upstream fit does not yet reject every
  unsupported plane; that separate gap remains in TASK.md.
...
work already done — and every settle of a slider paid it again. Measured on the original
...
| Private-review images and their exact provenance | `docs/evidence/` |

**Local-only evidence** — a fresh clone will not have these, and they must be regenerated by a
...
### The private-repository handoff passes from a clean clone — 2026-08-11

The code and dependency implementation ends at `5c7eac2`, the **96th retained commit**; no history
was rewritten. A third temporary sibling clone started from tracked files only. It used Node
26.4.0, npm 11.17.0, Python 3.12.13 and ffmpeg 8.1.2. `npm ci`, the pinned Python install, the
complete verifier and the production build passed. The verifier reported 37 test files passed and
3 fixture-only files skipped: **519 tests passed, 23 skipped**, followed by every FastAPI server
contract. The skipped files need the large door and room payloads intentionally absent from git.

The exact-head clone generated a 4.0 s, 640×360 clip, probed it, extracted 40 frames and sent four
of those JPEGs through the loopback mock endpoint. The reply said `mock: true`, returned a
four-frame manifest and named the expected DA3 model, L4 mock device and scene artifact. Browser
control was unavailable for that final repeat, so the immediately preceding clean clone supplied
the visible check: the same clip produced 40 frames, an explicit **MOCK** result and the fixture's
351,232 points while Cloud control read Project unconfigured, Service none and Cost none. The app
source did not change between those runs. The full 1280×800 design review also passed with zero
browser errors or warnings; its 28-item record is in `docs/design-review-log.md`.

The local control plane now binds to loopback. Privileged calls require both the same browser
origin and a random token created for that server process. Run identifiers, file paths, service
addresses and artifact addresses are checked before any process starts or identity token is made.
Hostile origin, missing-token, service-address, artifact and path cases are regression tests. The
cloud scripts require an explicit project, region and bucket; they use a dedicated runtime service
account and never change the operator's global gcloud project.

The retained tree and Git history were scanned for Google API keys, OAuth client secrets, refresh
tokens, private keys, credential files and common provider-token shapes. No real credential match
was found. Two deliberately fake Google-token strings remain in redaction tests. The history also
contains one author email and historical cloud project, bucket and execution identifiers; those
are publication metadata, not credentials, and the approved history was not rewritten. `.env`
files and Google CLI credentials are not present. `gcloud auth login` stores its account material
outside this repository.

The dependency audit is also clean at this gate. Transformers.js still requests Sharp 0.34,
although its browser export neither runs nor ships that Node-only image library. Sharp 0.34 carries
two high-severity libvips advisories, so the lockfile overrides it with the fixed 0.35.3 release.
A clean install, a raw-image Sharp smoke, the app build and `npm audit --omit=dev` passed with zero
vulnerabilities. Keep the override until Transformers.js moves its own dependency to the fixed
line.

Only macOS has completed this clean-clone exercise. The workflow also targets Ubuntu, but its first
hosted run remains evidence to collect rather than a claimed pass. WSL 2 is experimental. The real
Google preflight, permissions, deploy, signed bucket fetch, inference and teardown were not run in
this zero-cost review. Two comments in the frozen GPU server still say “Mac”; changing them alone
...
- **A pane squeezed below about 100 px tall cuts its own control stack off at the bottom.** Rows
  ...
  default layout.
- **Nothing above 112 frames at 504 px is a verified operating point.** The allocator peak at 112
  repeats at 17.23 GiB across two clips, while 128 and 144 frames leave roughly 1% driver headroom.
  The app keeps the 112-frame cap because finishing once is not the same as running safely.
