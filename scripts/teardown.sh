...
# Run artifacts are DELIBERATELY left in place: they now outlive the service, which is the
# whole reason the service can be deleted so freely. They are not left forever, though, and
# "not forever" is a claim worth showing rather than asserting -- so print what is actually
# sitting there and when the lifecycle rule will take it.
#
